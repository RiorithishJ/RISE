...
// Tracker Cards
const trackers = [
  { icon: Dumbbell, label: "Fitness", stat: "3 activities logged", sub: "420 kcal burned", progress: 65, section: "fitness" },
  { icon: Github, label: "GitHub", stat: "4 commits today", sub: "2 repos active", progress: 50, section: "github" },
  { icon: Briefcase, label: "LinkedIn", stat: "Post due Sunday", sub: "Weekly reminder", progress: 30, section: "linkedin" },
  { icon: Timer, label: "Pomodoro", stat: "3 sessions today", sub: "75 min focused", progress: 75, section: "pomodoro" },
];
...
const DashboardPage = ({ onNavigate }: DashboardPageProps) => {
  ...
  const addIdea = () => {
    ...
    saveIdeas([...postIdeas, { id: Date.now(), topic: newIdea, date: new Date().toLocaleDateString("en-US", { month: "short", day: "numeric" }), status: "Planned" }]);
    setNewIdea("");
  };
  ...
  const handleCardClick = (tracker: typeof trackers[0]) => {
    if (tracker.label === "LinkedIn") {
      ...
      onNavigate("progress", tracker.section);
    }
  };
  ...
  return (
    <div className="flex-1 overflow-y-auto p-8">
        ...
        {trackers.map((t) => {
          ...
          return (
            ...
            >
              ...
              {isLinkedin ? (
                <button
                  onClick={(e) => { e.stopPropagation(); setLinkedinActive(!linkedinActive); }}
        ...
        <div className="card-rise flex-[2] flex flex-col min-h-[250px]">
              ...
              <div key={task.id} className="flex items-center gap-2 group">
                <button
                  ...
                  className={`w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 transition-colors ${
                    task.done ? "bg-primary border-primary" : "border-gray-300"
                  }`}
                >
                  {task.done && <Check size={12} className="text-primary-foreground" />}
                </button>
                <span className={`text-sm flex-1 ${task.done ? "line-through text-muted-foreground" : "text-rise-text"}`}>
                  {task.text}
                </span>
          ...
          <div className="flex gap-2 mt-3">
            <input
              ...
              className="flex-1 text-sm bg-muted rounded-lg px-3 py-2 outline-none text-rise-text placeholder:text-muted-foreground"
            />
      ...
      {/* LinkedIn Slide Panel */}
      {linkedinPanel && (
        <div className="fixed inset-0 z-50 flex justify-end" onClick={() => setLinkedinPanel(false)}>
          <div className="absolute inset-0 bg-black/30" />
          <div
            className="relative w-[400px] h-full bg-card overflow-y-auto animate-in slide-in-from-right duration-300"
              ...
              {linkedinActive && (
                <>
                  ...
                  <div className="rounded-2xl p-4 bg-primary/10 border border-primary/20">
                    ...
                    <button className="text-sm bg-primary text-primary-foreground px-4 py-1.5 rounded-lg font-medium hover:opacity-90 transition-opacity">
                      Save this idea
                    </button>
                  </div>
                </>
